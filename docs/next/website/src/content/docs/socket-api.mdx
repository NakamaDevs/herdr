...
`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this submits the prompt and starts the wait in one request, avoiding a race between separate calls. If the resolved agent is already `blocked`, `agent.prompt` returns `agent_blocked` without sending input or starting the wait. `agent.prompt` also accepts an optional `submit` boolean. Omitted or `true` sends the text and then a delayed Enter. `false` applies the same target and readiness guards and the same bracketed-paste encoding, sends the text, and never schedules Enter, so staged text is only submitted by a later explicit action such as `agent.send_keys` with `enter`. The response is the same `agent` record. If the same pane still has a delayed Enter pending from an earlier `agent.prompt`, the request returns `agent_prompt_submit_pending` without sending text; the earlier Enter is left to deliver, and the caller can retry afterwards. `submit: false` cannot be combined with `wait`; that request returns `agent_prompt_wait_requires_submit` before any input is sent.
